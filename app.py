...
def main():
    ...
    try:
        ...
        if pool_water_path.exists():
            ...
            st.markdown(f'''
            <script>
            // Wait for page to fully load, then add persistent pool water background
            function addPoolWaterBackground() {{
                // Remove any existing pool water divs
                const existingBg = document.getElementById('pool-water-persistent');
                if (existingBg) {{
                    existingBg.remove();
                }}
                
                // Create new pool water background div
                const poolDiv = document.createElement('div');
                poolDiv.id = 'pool-water-persistent';
                ...
                // Keep running this to maintain background
                setTimeout(addPoolWaterBackground, 2000);
            }}
            
            // Start when page loads and keep running
            if (document.readyState === 'loading') {{
                document.addEventListener('DOMContentLoaded', addPoolWaterBackground);
            }} else {{
                addPoolWaterBackground();
            }}
            </script>
            ''', unsafe_allow_html=True)
        else:
