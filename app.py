...
def main():
    ...
    # Add pool water background directly as HTML
    try:
        pool_water_path = Path(__file__).parent / 'pool_water.jpg'
        if pool_water_path.exists():
            with open(pool_water_path, 'rb') as f:
            ...
            # Debug fallback - show a visible indicator
            st.markdown('''
            <div style="
                position: fixed;
                top: 0;
    ...
    with col_logo:
        # Try to load and display pool water image first as a test
        try:
            pool_water_path = Path(__file__).parent / 'pool_water.jpg'
            if pool_water_path.exists():
                st.image(str(pool_water_path), width=200, caption="Pool Water (Test)")
                st.success("✅ Pool water image found and loaded!")
            else:
                st.error("❌ pool_water.jpg not found in folder")
                st.info(f"Looking in: {Path(__file__).parent}")
        except Exception as e:
            st.error(f"Error loading pool water: {str(e)}")
