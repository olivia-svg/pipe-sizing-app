...
def main():
    ...
    st.markdown(
        """
        <style>
        /* lighter water-blue background and darker text         with col_a:
    ...
    # Status text centered under the table - OUTSIDE the column structure
    st.markdown(f"", unsafe_allow_html=True)  .stApp { background-color: #e8f7ff; color: #03263a; }
    ...
    # Status text centered under the table with background box
    st.markdown(f"""
    <div style='width: 100%; text-align: center; margin: 20px auto; padding: 12px; 
                background-color: #f0f8ff; border: 2px solid #0b82bf; border-radius: 8px; 
                max-width: 600px; font-size: 16px;'>
        <strong>Status:</strong> {badge_label} &nbsp;•&nbsp; <em>Limit used:</em> {active_limit} ft/s
    </div>
    """, unsafe_allow_html=True)
    
    # Status text centered under the table - COMPLETELY SEPARATE
    st.markdown("<div style='width: 100%; text-align: center; margin-top: 20px; padding: 10px; background-color: #f0f8ff; border-radius: 8px;'><strong>Status:</strong> {badge_label} &nbsp;•&nbsp; <em>Limit used:</em> {active_limit} ft/s</div>".format(badge_label=badge_label, active_limit=active_limit), unsafe_allow_html=True)
    ...
    with st.container():
        ...
        with col_a:
            ...
            if recommended_size and recommended_velocity:
                ...
                st.markdown(f"<div class='kpi-badge' style='background:{badge_color}'> None</div>", unsafe_allow_html=True)
        with col_b:
            # Center the status text under the table
            st.markdown(f"<div style='text-align: center; margin-top: 10px;'><strong>Status:</strong> {badge_label} &nbsp;•&nbsp; <em>Limit used:</em> {active_limit} ft/s</div>", unsafe_allow_html=True)
